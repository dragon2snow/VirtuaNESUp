...
//		DEBUGOUT( "Inactivate.\n" );
			m_bActivate = FALSE;
			// ÉoÉbÉNÉOÉâÉEÉìÉhìÆçÏONÇ©ÉlÉbÉgÉvÉåÉCíÜÇÕÉ|Å[ÉYÇµÇ»Ç¢
...
//		DEBUGOUT( "Activate.\n" );
		m_bActivate = TRUE;
		// ÉoÉbÉNÉOÉâÉEÉìÉhìÆçÏONÇ©ÉlÉbÉgÉvÉåÉCíÜÇÕÉåÉWÉÖÅ[ÉÄÇµÇ»Ç¢
		if( !(Config.emulator.bBackground || NetPlay.IsConnect()) ) {
			Emu.Resume();
		}
		// DirectInputóLå¯
...
//	DEBUGOUT( "CMainFrame::OnNetPlayChatPopup\n" );
	...
	if( Config.general.bScreenMode ) {
		...
		// É_ÉCÉAÉçÉOÇ™âÊñ äOÇ»ÇÁÇŒíÜâõÇ…à⁄ìÆÇ∑ÇÈ:)
		RECT	rcParent, rc;
...
DEBUGOUT( "ÉXÉeÅ[ÉgÉtÉ@ÉCÉã CHK=%d\n", ret );
...
DEBUGOUT( "ÉÄÅ[ÉrÅ[ÉtÉ@ÉCÉã CHK=%d\n", ret );
...
//	DEBUGOUT( "CMainFrame::OnLanguageCfg\n" );
	...
	if( dlg.DoModal( m_hWnd ) == IDOK ) {
		...
		// ÉTÅ[É`É_ÉCÉAÉçÉOÇÕï¬Ç∂ÇÈ
		if( m_SearchDlg.m_hWnd && ::IsWindow(m_SearchDlg.m_hWnd) ) {
			m_SearchDlg.Destroy();
		}

		// ÉoÅ[ÉRÅ[Éhì¸óÕÉ_ÉCÉAÉçÉOÇÕï¬Ç∂ÇÈ
...
void PpuDump(void)
{
	...
	if(fp==NULL){
		MessageBox(NULL,"DUMP PPU ≥ˆ¥Ì","≥ˆ¥Ì",0);
		return;}
...
void CpuDump(void)
{
	...
	FILE *fp=fopen(exePath,"w+b");
	if(fp==NULL){
		MessageBox(NULL,"DUMP CPU ≥ˆ¥Ì","≥ˆ¥Ì",0);
		return;}
...
BYTE	KeyInp[256+64*8];
	...
	// 20msñ¢ñûÇ≈ÇÕéÛÇØïtÇØÇ»Ç¢ÇÊÇ§Ç…Ç∑ÇÈ
	...
	if( !m_bKeyEnable ) {
		// ÉgÉäÉKÇî≠ê∂Ç≥ÇπÇ»Ç¢ÇÊÇ§Ç…Ç∑ÇÈà◊
...
void _cdecl CMainFrame::KeyThreadProc( LPVOID lpParam )
{
	while( !m_bKeyThreadExit ) {
		...
		// ÉGÉ~ÉÖÉåÅ[ÉVÉáÉìíÜà»äOÇ±Ç¡ÇøÇ≈Ç‚ÇÁÇ»Ç¢
		if( Emu.IsRunning() ) {
			continue;
		} else {
			DirectInput.Poll();
